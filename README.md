...
Постарался по максимуму следовать пунктам задания. Единственное, не до конца понятен пункт 2 (раздел ajax). Непонятно каким методом должен отправляться запрос и нужно ли передавать в нём данные из формы. Учитывая, что в задании эти моменты упущены - отправляю GET запрос на JSON-файлы в каталоге responses без передачи данных. Также при валидации ФИО сделал "ровно 3 слова" с дополнительной возможностью использования символа "-" для составных фамилий, имён и отчеств.
